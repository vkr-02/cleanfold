...
const Dashboard = (props) => {
  ...
  useEffect(() => {
    fetch(
      'http://localhost:5000/orders?email=' + auth.user.email
    )
      .then((res) => res.json())
  ...
  return (
    <Container>
      <div className="d-flex justify-content-between align-items-center mb-4 mt-5">
        <h2 className="text-danger head-title mb-0">Your Order History</h2>
        <Button 
          color="danger" 
          size="lg" 
          className="px-4 py-2 font-weight-bold text-uppercase" 
          style={{ borderRadius: '50px', boxShadow: '0 5px 15px rgba(239, 68, 68, 0.4)', letterSpacing: '1px' }}
      ...
      <Modal
        ...
        ariaHideApp={false}
      >
        <FeedbackForm closeModal={() => setFeedbackModalIsOpen(false)} />
      </Modal>
